...
internal inline void * heap_alloc(heap_t *heap, u64 n_bytes) {
     ...
     * always aligned to at least the minimum required alignment.
     */
...
out:;
    ASSERT(mem != NULL,        "did not get memory");
    ASSERT(IS_ALIGNED(mem, 8), "user memory is not properly aligned for performance");
...
internal inline void * _heap_aligned_alloc(heap_t *heap, size_t n_bytes, size_t alignment) {
    u64              sblock_n_bytes;
    u64              cblock_n_bytes;
    u64              size_class;
    u32              size_class_idx;
    ...
    cblock_n_bytes = MIN(alignment + n_bytes, 2 * MAX(alignment, n_bytes));
    ...
    ASSERT(size_class_idx < N_SIZE_CLASSES, "invalid size-class index");
    ...
    chunk_end    = ((void*)chunk) + sizeof(chunk_header_t) + CHUNK_SIZE(chunk);
    ...
    *chunk      = *split_chunk;
    ...
    ASSERT(CHUNK_SIZE(split_chunk) >= CHUNK_MIN_SIZE, "split chunk is too small");

    chunk->flags = 0;

    cblock_free_chunk((cblock_header_t*)ADDR_PARENT_BLOCK(split_chunk), split_chunk);

    ASSERT(CHUNK_SIZE(chunk) >= n_bytes, "chunk is too small");

    ASSERT(aligned_addr >= ((void*)chunk) + sizeof(chunk_header_t),
                                                "aligned memory starts before its chunk");
    ASSERT(chunk > split_chunk,                 "the chunks split goofy");
...
internal inline void * heap_aligned_alloc(heap_t *heap, size_t n_bytes, size_t alignment) {
    ...
    mem = _heap_aligned_alloc(heap, n_bytes, alignment);
